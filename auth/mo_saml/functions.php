...
function auth_mo_saml_authenticate_user_login($accountmatcher, $userssaml, $samlcreate=false, $samlupdate=false) {
    ...
    if (!$user->id) {
	    ...
	    //user enrollments and mapping company to user
            $userId = $user->id;
            $course_id = $DB->get_record('course', array('shortname' => "CC"))->id;
            ...
            $companyid = 3;
            $departmentid = $DB->get_record('department', array('company' => $companyid, 'parent' => 0))->id;
            $data = new stdClass();
            $data->userid = $userId;
            $data->companyid = $companyid;
            $data->departmentid = $departmentid;
            $data->managertype = 0;
            $result = $DB->insert_record('company_users', $data);
            //ends here..
