...
function user_get_user_navigation_info($user, $page, $options = array()) {
   ...
   // $returnobject->navitems[] = $myprofile;
    ...
    if (is_siteadmin())
    {
        $logincms = new stdClass();
        $logincms->itemtype = 'link';
        $logincms->url = new moodle_url('/local/custompage/logincms.php');
        $logincms->title = 'Login CMS';
        $logincms->titleidentifier = 'custompage,logincms';
        $logincms->pix = "i/grades";
        $returnobject->navitems[] = $logincms;
    }
