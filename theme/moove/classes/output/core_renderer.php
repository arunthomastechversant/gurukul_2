...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    // take logos 
    public function get_quest_logo() {
        ...
        if ($files = $fs->get_area_files($context->id, 'local_custompage', 'questlogo',false, 'sortorder', false)) 
        {
            ...
            {
                $logopath = moodle_url::make_pluginfile_url($context->id, 'local_custompage', 'questlogo', $file->get_itemid(), $file->get_filepath(), $file->get_filename());
                $logopath = $logopath->__toString();
            }
            
        }
        return $logopath;
    }
    public function get_gurukul_logo() {
        $context = context_system::instance();
        $fs = get_file_storage();
        if ($files = $fs->get_area_files($context->id, 'local_custompage', 'gurukullogo',false, 'sortorder', false)) 
        {
            foreach ($files as $key => $file) 
            {
                $logopath = moodle_url::make_pluginfile_url($context->id, 'local_custompage', 'gurukullogo', $file->get_itemid(), $file->get_filepath(), $file->get_filename());
                $logopath = $logopath->__toString();
            }
    ...
    public function get_loginbgimg() {
        $context = context_system::instance();
        $fs = get_file_storage();
        if ($files = $fs->get_area_files($context->id, 'local_custompage', 'loginbgimg',false, 'sortorder', false)) 
        {
            foreach ($files as $key => $file) 
            {
                $loginbgimg = moodle_url::make_pluginfile_url($context->id, 'local_custompage', 'loginbgimg', $file->get_itemid(), $file->get_filepath(), $file->get_filename());
                $loginbgimg = $loginbgimg->__toString();
            }
