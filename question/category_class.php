...
class question_category_object {
    ...
    public function add_category($newparent, $newcategory, $newinfo, $return = false, $newinfoformat = FORMAT_HTML,
        ...
        $companydata=$DB->get_record_sql("select cc.*,c.name,c.shortname,co.id as coid from {company_course} cc JOIN mdl_company c ON c.id = cc.companyid JOIN mdl_context co where cc.courseid=$courseid AND co.instanceid=$courseid AND co.contextlevel=50 ");
       
        if($companydata->shortname == 'rsl'){
            $paren=$DB->get_record_sql("select * from {question_categories} where name='top' and contextid=$companydata->coid");
            if($paren){
                $subparen=$DB->get_record_sql("select * from {question_categories} where name !='top' and parent=$paren->id");
            }
            if($subparen->id != $parentid  ){


                $data = new stdClass();
                $data->fullname = $newcategory;
                $data->shortname = $newcategory;
                $data->selfenrol = 0;
                $data->summary_editor = '';
                $data->startdate = time();
                $data->enddate = 0;
                $data->submitbutton = 'Create course';
                $data->title = '';
                $data->description = '';
                $data->userid = $USER->id;
                ...
                $course = create_course($mergeddata, $editoroptions);

                $quest = new stdClass();
                $quest->courseid = $course->id;
                $quest->question_categoryid = $categoryid;
                $quest->status = 1;

                $categoryid = $DB->insert_record("question_category_mapping", $quest);
