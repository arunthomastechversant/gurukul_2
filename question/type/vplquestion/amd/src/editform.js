...
'qtype_vplquestion/scriptsloader'], function($, jqui, log, VPLService, CodeEditors, ScriptsLoader) {
    ...
    function updateExecfilesVisibility() {
        ...
        $('.novplmessage')[selectedVpl ? 'hide' : 'show']();
        $('#execfileslist, #fitem_id_execfile')[selectedVpl ? 'show' : 'hide']();

        $('#fitem_id_precheckexecfileslist')[showPrecheckExecfiles ? 'show' : 'hide']();
        $('#precheckexecfileslist, #fitem_id_precheckexecfile')[selectedVpl && showPrecheckExecfiles ? 'show' : 'hide']();
    ...
    function applyTemplateChoice(keepContents) {
        var selectedVpl = $('#id_templatevpl').val();
        $('#fitem_id_templatecontext')[selectedVpl ? 'show' : 'hide']();
    ...
    function updateExecfile($prevFile, $newFile, aceEditor, $hiddenField) {
        ...
        if ($prevFile !== null) {
            ...
            $prevFile[(prevFileContent.startsWith('UNUSED') ? 'add' : 'remove') + 'Class']('unused');
    ...
    return {
        setup: function(aceTheme, templateChangeHelpButton) {
            // Setup all form editors.
            CodeEditors.setupFormEditors(aceTheme, function() {
                ScriptsLoader.loadVPLUtil(function() {
