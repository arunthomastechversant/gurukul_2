...
class create_lead_test_form extends moodleform {

function definition() {
    global $DB,$CFG,$PAGE,$USER;
        $mform = $this->_form;
        $qid  = optional_param('qid', '', PARAM_TEXT);
        $courseid  = optional_param('qid', '', PARAM_TEXT);
    ...
    // $courses = $DB->get_records('company_course',array('companyid' => $companyid));
    ...
    $rr=array();
    $cr=array();
    $rr=array(''=>'---- Choose Question Category ----');
    $cr=array(''=>'---- Choose Course ----');
    // foreach($categories as $key => $val){
    //         $rr[$val->id] = $val->name;
    // } 
    foreach($courses as $ckey => $cval){
            $cr[$cval->id] = $cval->fullname;
    }   
        //  echo '<pre>';print_r($categories);exit;

          $size='size="20"';
          $mform->addElement('header', 'createtest', 'Create Lead Test');
          $select = $mform->addElement('select', 'course', 'Choose a Course', $cr);
          $mform->addRule('course', 'Select a Course', 'required', null, 'client');

          $mform->addElement('select', 'question_category', 'Choose category', $rr);
