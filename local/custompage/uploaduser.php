...
require_once($CFG->dirroot.'/local/custompage/uploaduser_form.php');
...
$linktext = "Upload Lead User";
// Set the url.
$linkurl = new moodle_url('/local/custompage/uploaduser.php');
...
$returnurl = $CFG->wwwroot."/local/custompage/uploaduser.php";
...
$stdfields = array('id', 'firstname', 'lastname', 'username', 'email',
        ...
        'password', 'temppassword', 'suspended','role');
...
if ($mform->is_cancelled()) {
 ...
 // Start of lead section
                $leadUser = new stdClass();
                $leadUser->userid = $user->id;
                $leadUser->username = $user->username;
                $leadUser->password = $user->newpassword;
                $leadUser->email = $user->email;
                $leadUser->role = $user->role;
// print_r($leadUser);exit();

                $u1 = $DB->insert_record('lead_user_detail', $leadUser);
