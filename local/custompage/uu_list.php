...
$PAGE->set_title("URDC Users List");
$PAGE->set_heading("URDC Users List");
$PAGE->set_url($CFG->wwwroot.'/local/custompage/uu_list.php');
$PAGE->navbar->add('URDC Users List', new moodle_url('/local/custompage/uu_list.php'));
require_login();
echo $OUTPUT->header();
$PAGE->set_context(context_system::instance());

$baseurl = new moodle_url('/local/custompage/uu_list.php');
$i=1;
$datas = $DB->get_records_sql("SELECT * FROM {urdc_user_detail} order by id DESC");
// print_r($datas);exit;
        ...
        foreach($datas as $data){
            ...
            $drive = $DB->get_record_sql("SELECT * FROM {urdc_recruitment_drive} where id=$data->recruitment_id");      
...
$add_organization = $CFG->wwwroot.'/local/custompage/create_uu_form.php';

echo '<a href="'.$add_organization .'" class="btn btn-secondary" >Add URDC Users</a></br></br>';
