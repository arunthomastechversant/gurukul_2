...
require_once('assign_lu_form.php');
?>
...
$PAGE->set_url($CFG->wwwroot.'/local/custompage/assign_lu.php');
$PAGE->navbar->add('Assign Lead User', new moodle_url('/local/custompage/assign_lu.php'));
require_login();
echo $OUTPUT->header();
$PAGE->set_context(context_system::instance());

$baseurl = new moodle_url('/local/custompage/assign_lu.php');
...
$users = $DB->get_records_sql("SELECT u.* FROM  {company_users} as cu JOIN {user} as u WHERE cu.userid = u.id AND cu.companyid = $companyid");
if(count($users) >=1){
...
// print_r($user);
            ...
            $type = "";
                ...
                $trainerRoleId = $DB->get_record('role', array('shortname' => 'lt'))->id;
                $studentRoleId = $DB->get_record('role', array('shortname' => 'student'))->id;
                if($roleId == $trainerRoleId){
                    $type = "Trainer";
                }
                else if($roleId == $studentRoleId){
                    $type = "Student";
                }else{}
            }
            ...
            // if(isset($roleId)){
            //     if($roleId == $trainerRoleId){
            //         $type = "<span class='label label-danger'>Trainer</span>";
            //     }
            //     else if($roleId == $studentRoleId){
            //         $type = "<span class='label label-danger'>Student</span>";
            //     }
            // }else{
            //     $type = "";
            // }
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        var quotations = [];
        oTable = $('#lead_user_list').DataTable( {

        initComplete: function () {
            var table = $('#lead_user_list').DataTable();
            this.api().columns().every( function () {
                var column = this;

                if (column.index() == 5) {
                    $('<span style="margin-left: 10px;"></span>   ').appendTo( '#lead_user_list_length' );
                        var select = $('<select id="class_select" class="custom-select"></select>')
                        .appendTo( '#lead_user_list_length' )
                        .on( 'change', function () {
...
$('#id_assign_lu').click(function(){
    ...
    var course_id = document.getElementById("id_shortname").value;
    // var role_type = $("id_roletype_group").value;
    ...
    var role = "";
        ...
        var enrol_type = '';
    $("#lead_user_list input[name=assin]:checked").each(function () {
        values.push(this.id);
        var userCount = values.length;
        if(userCount > 1){
           enrol_type = 'bulk';
        }
        else{ enrol_type = 'single';}
    });
        // alert(role_type);
    if(values == ""){
