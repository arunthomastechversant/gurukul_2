...
require_once($CFG->dirroot.'/local/custompage/urdcbulkupload_form.php');
...
$linktext = "Upload URDC Users";
// Set the url.
$linkurl = new moodle_url('/local/custompage/urdcbulkupload.php');
...
$returnurl = $CFG->wwwroot."/local/custompage/urdcbulkupload.php";
...
// Array of all valid fields for validation.
    ...
    if ($formdata = $mform->get_data()) {
        ...
        while ($line = $cir->next()) {
            ...
            if(!$userid){
                $drive=$DB->get_record_sql("select *  from {urdc_recruitment_drive} where name = '$user->drive'");
                if($drive){
                    // print_r($drive);exit();
                    $data='';
