...
$PAGE->set_url($CFG->wwwroot.'/local/custompage/brm_drive_list.php');
$PAGE->navbar->add('BT Score List', new moodle_url('/local/custompage/bu_drive_list.php'));
// require_login();
echo $OUTPUT->header();
$PAGE->set_context(context_system::instance());

$baseurl = new moodle_url('/local/custompage/brm_drive_list.php');
$page = optional_param('page', 0, PARAM_INT);
$perpage = optional_param('perpage', 10, PARAM_INT);

$t_count = count($DB->get_records_sql("SELECT rd.id,rd.name as drive_name,q.name as test,q.timelimit FROM {bt_recruitment_drive} as rd INNER JOIN {bt_user} as bu INNER JOIN {quiz} as q where rd.test = q.id"));
...
$datas = $datas = $DB->get_records_sql("SELECT rd.id,rd.name as drive_name,q.name as test,q.timelimit FROM {bt_recruitment_drive} as rd INNER JOIN {bt_user} as bu INNER JOIN {quiz} as q where rd.test = q.id",array(),$start, $perpage);
if(count($datas) >=1){
        foreach($datas as $data){

            $users_url=$CFG->wwwroot."/local/custompage/brm_user_list.php?drive_id=".$data->id;
