...
$PAGE->set_title("Add BT User");
$PAGE->set_heading("Add BT User");
$PAGE->set_url($CFG->wwwroot.'/local/custompage/create_bu_form.php');
$coursenode = $PAGE->navbar->add('Add BT User', new moodle_url($CFG->wwwroot.'/local/custompage/create_bu_form.php'));
$PAGE->set_context(context_system::instance());
//~ $url = $CFG->wwwroot.'/local/custompage/get_data.php';
$return = $CFG->wwwroot.'/local/custompage/bu_list.php';
echo $OUTPUT->header();

$mform = new create_bu_form();
if ($mform->is_cancelled()) {
 ...
 // $rec->rsl_recruiter = $record->rsl_recruiter;	
    ...
    $DB->insert_record('bt_user', $rec);


for ($x = 1; $x <= $record->nofu; $x++) {
    $data='';
    $userdata = new stdClass();
    $userdata->firstname = 'BT';
    $userdata->lastname = time();
    $userdata->username = 'bt'.rand(10,100).password_generate(6);
    $userdata->userdepartment=$departmentid; 
    $userdata->email='bt'.rand(10,100).time().'@gmail.com'; 
 
    $userdata->newpassword='Bt@1'.password_generate(4);
	$systemcontext = context_system::instance();
	$companyid = iomad::get_my_companyid($systemcontext);
    ...
    $enrol = enrol_get_plugin('manual');
        ...
        $record1->bt_due=  '';
        $lastinsertid = $DB->insert_record('bt_user_detail', $record1);
    ...
    $urlto = $CFG->wwwroot.'/local/custompage/bu_list.php';
