    ...
    foreach($idlist as $key => $value){
        ...
        if($enrolled_users){
            ...
            if($proctoring == 1){
                ...
                $mouth_open_count = $DB->get_record_sql("SELECT count(mouth_open_count) as total_count from {proctoringdetails} where userid = $value and quizid = $enrolled_users->cmid and mouth_open_count = 1 ");
                $mobile_phone_count = $DB->get_record_sql("SELECT count(mobile_phone_count) as total_count from {proctoringdetails} where userid = $value and quizid = $enrolled_users->cmid and mobile_phone_count = 1 ");
                $more_person = $DB->get_record_sql("SELECT count(more_person) as total_count from {proctoringdetails} where userid = $value and quizid = $enrolled_users->cmid and more_person = 1 ");
                $no_person = $DB->get_record_sql("SELECT count(no_person) as total_count from {proctoringdetails} where userid = $value and quizid = $enrolled_users->cmid and no_person = 1 ");
                $head_up = $DB->get_record_sql("SELECT count(head_up) as total_count from {proctoringdetails} where userid = $value and quizid = $enrolled_users->cmid and head_up = 1 ");
                $head_down = $DB->get_record_sql("SELECT count(head_down) as total_count from {proctoringdetails} where userid = $value and quizid = $enrolled_users->cmid and head_down = 1 ");
                $head_left = $DB->get_record_sql("SELECT count(head_left) as total_count from {proctoringdetails} where userid = $value and quizid = $enrolled_users->cmid and head_left = 1 ");
                $head_right = $DB->get_record_sql("SELECT count(head_right) as total_count from {proctoringdetails} where userid = $value and quizid = $enrolled_users->cmid and head_right = 1 ");
                $face_recognition = $DB->get_record_sql("SELECT count(face_recognition) as total_count from {proctoringdetails} where userid = $value and quizid = $enrolled_users->cmid and face_recognition = 1 ");
...
foreach($mail_ids as $mailid){
    ...
    $drivedate = date('d/m/Y', $DB->get_record('bt_recruitment_drive',array('id' => $drive_id))->startdate);
