...
$PAGE->set_title("Assign Lead Student");
$PAGE->set_heading("Assign Lead Student");
$PAGE->set_url($CFG->wwwroot.'/local/custompage/assign_lu.php');
$PAGE->navbar->add('Assign Lead Student', new moodle_url('/local/custompage/assign_lu.php'));
require_login();
echo $OUTPUT->header();
...
$users = $DB->get_records_sql("SELECT u.* FROM  {user} as u 
    ...
    WHERE lu.userid = u.id AND lu.role = 'student'");
if(count($users) >=1){
        foreach($users as $user){
// print_r($user);
            $roleId = $DB->get_record('role_assignments', array('userid' => $user->id))->roleid;
            if(!($DB->record_exists('lead_batch_user_assignments', array('userid' => $user->id)))){
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        var quotations = [];
        oTable = $('#lead_student_list').DataTable( {

        initComplete: function () {
            var table = $('#lead_student_list').DataTable();
            this.api().columns().every( function () {
                var column = this;

                if (column.index() == 25) {
                    $('<span style="margin-left: 10px;"></span>   ').appendTo( '#lead_student_list_length' );
                        var select = $('<select id="class_select" class="custom-select"></select>')
                        .appendTo( '#lead_student_list_length' )
                        .on( 'change', function () {
...
$('#id_assign_lu').click(function(){
    ...
    $("#lead_student_list input[name=assin]:checked").each(function () {
        values.push(this.id);
    });
    if(values == ""){
