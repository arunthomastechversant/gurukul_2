...
$url=$CFG->wwwroot.'/local/custompage/create_test_form.php';
if ($deleteid) {
	    	           
    $result= $DB->delete_records('ru', array('id'=>$deleteid));     
     redirect(new moodle_url('/local/custompage/test_list.php'),'recruitment_drive Deleted Successfully', 3);
}
...
if($updateid){
   $update = $DB->get_record('ru', array('id'=>$updateid), '*', MUST_EXIST);
    ...
    if ($update) {
       ...
       redirect(new moodle_url('/local/custompage/test_list.php'),'recruitment_drive Updated Successfully', 3);
   }

}

$PAGE->set_pagelayout('admin');
$PAGE->set_title("Create RSL Test");
$PAGE->set_heading("Create RSL Test");
$PAGE->set_url($CFG->wwwroot.'/local/custompage/create_test_form.php');
$coursenode = $PAGE->navbar->add('Create RSL Test', new moodle_url($CFG->wwwroot.'/local/custompage/create_test_form.php'));
$PAGE->navbar->add('Add RSL Recruitment Drive', new moodle_url($CFG->wwwroot.'/local/custompage/create_rsl_recruitment_drive_form.php'));
...
$mform = new create_test_form();
if ($mform->is_cancelled()) {
    redirect($return);

} else if ($record = $mform->get_data()) {
    // print_r($_REQUEST['category']);exit;
    // print_r($record);exit();
    $data_product=array();
    ...
    // $remainder=$questions % count($question_category);
    // $quotent=$questions / count($question_category);

    // print_r($question_category);exit;
	...
	// 	$quizdata->attemptimmediately = 0;
	// 	$quizdata->correctnessimmediately = '';
	// 	$quizdata->marksimmediately = 0;
	// 	$quizdata->specificfeedbackimmediately = 0;
	// 	$quizdata->generalfeedbackimmediately = 0;
	// 	$quizdata->rightanswerimmediately = 0;
	// 	$quizdata->overallfeedbackimmediately = 0;
	// 	$quizdata->attemptopen = 0;
	// 	$quizdata->correctnessopen = '';
	// 	$quizdata->marksopen = 0;
	// 	$quizdata->specificfeedbackopen = 0;
	// 	$quizdata->generalfeedbackopen = 0;
	// 	$quizdata->rightansweropen = 0;
	// 	$quizdata->overallfeedbackopen = 0;
    ...
    $quizdata->competency_rule = 0;
    //edit
    // $quizdata->reviewmarks = 0;
    // $quizdata->reviewspecificfeedback = 0;
    // $quizdata->reviewgeneralfeedback = 0;
    // $quizdata->reviewrightanswer = 0;
    $quizdata->submitbutton2 = 'Save and return to course';
    ...
    //     for ($x = 0; $x < count($question_category); $x++) {
    //         $quotent=$questions / count($question_category);

    //         $remainder=$questions % count($question_category);
    //         quiz_add_random_questions($quiz, 1, $question_category[$x], $quotent, $includesubcategories, $tagids);   //  $quotent -> count of questions
           
    //    }
    //    if($remainder > 0){
    //         // $question_category[0]  -- remainder number of questions
    //         quiz_add_random_questions($quiz, 1, $question_category[0], $remainder, $includesubcategories, $tagids); 
    ...
    $urlto = $CFG->wwwroot.'/local/custompage/create_test_form.php';
