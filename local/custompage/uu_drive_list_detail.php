...
$PAGE->set_heading('URDC User Scores'); 
$PAGE->navbar->add('URDC User Scores', new moodle_url($CFG->wwwroot.'/local/custompage/uu_drive_list_detail.php'));

$drive_id = optional_param('drive_id', '', PARAM_RAW);

$page = optional_param('page', 0, PARAM_INT);
$perpage = optional_param('perpage', 10, PARAM_INT);
$PAGE->set_url('/local/custompage/uu_drive_list_detail.php', array('drive_id' => $drive_id));
$data='';
require_login();

//admin_externalpage_setup('reportrslscore', '', null, '', array('pagelayout'=>'report'));
echo $OUTPUT->header();
$baseurl = $CFG->wwwroot."/local/custompage/uu_drive_list_detail.php.php?drive_id=".$drive_id;
$assi = $CFG->wwwroot."/local/custompage/uu_drive_list_detail.php?drive_id=".$drive_id;

if(is_siteadmin() || user_has_role_assignment($USER->id, 10)){
 ?>
    <a href="<?php echo $CFG->wwwroot;?>/local/custompage/uu_drive_list.php" class="btn btn-primary"> Back </a> 
...
<?php
    ...
    //  print_r($enrolled_users);exit; 
        foreach($datas as $key => $value){
            $enrolled_users = $DB->get_record_sql("select mqa.id as attemptid,u.firstname,u.lastname,u.email,uu.userid,mqa.sumgrades,qz.sumgrades as sumg from mdl_urdc_user_detail uu JOIN mdl_user u ON u.id=uu.userid JOIN mdl_groups g 
            ON g.id=uu.test_groupid JOIN mdl_urdc_recruitment_drive brd ON brd.id=uu.recruitment_id join mdl_quiz_attempts mqa on mqa.userid = uu.userid join {quiz} as qz on qz.id = mqa.quiz
