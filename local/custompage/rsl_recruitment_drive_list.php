...
$PAGE->set_url($CFG->wwwroot.'/local/custompage/rsl_recruitment_drive_list.php');
$PAGE->navbar->add('RSL recruitment drive List', new moodle_url('/local/custompage/create_rsl_recruitment_drive_form.php'));
$PAGE->navbar->add('Add RSL User', new moodle_url($CFG->wwwroot.'/local/custompage/create_ru_form.php'));
require_login();
echo $OUTPUT->header();
$PAGE->set_context(context_system::instance());

$baseurl = new moodle_url('/local/custompage/rsl_recruitment_drive_list.php');
...
$datas = $DB->get_records_sql("SELECT * FROM {rsl_recruitment_drive} ORDER by id DESC ",array(),$start, $perpage);
$actions = '';
...
if(count($datas) >=1){
        foreach($datas as $data){
            ...
            //delete button
            $buttons[] = html_writer::link(new moodle_url($CFG->wwwroot . '/local/custompage/create_rsl_recruitment_drive_form.php?deleteid='.$data->id), 
...
$add_recruitment_drive = $CFG->wwwroot.'/local/custompage/create_rsl_recruitment_drive_form.php';
