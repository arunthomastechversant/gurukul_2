...
$PAGE->set_title($company." User Report");
$PAGE->set_heading($company.' User Report'); 
$PAGE->navbar->add($company.' User Report', new moodle_url($CFG->wwwroot.'/local/custompage/usersreport.php'));

$drive_id = optional_param('drive_id', '', PARAM_RAW);

$page = optional_param('page', 0, PARAM_INT);
$perpage = optional_param('perpage', 10, PARAM_INT);
$PAGE->set_url('/local/custompage/usersreport.php', array('drive_id' => $drive_id));
$data='';
require_login();
        ...
        foreach($datas as $key => $value){
            ...
            if($enrolled_users){
                ...
                if($proctoring == 1){
                    ...
                    $proctoringdata = $DB->get_records_sql("SELECT * from {proctoringdetails} where userid = $value->id and quizid = $enrolled_users->cmid ");
