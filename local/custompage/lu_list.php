...
$PAGE->set_title("Lead Users List");
$PAGE->set_heading("Lead Users List");
$PAGE->set_url($CFG->wwwroot.'/local/custompage/ru_list.php');
$PAGE->navbar->add('Lead Users List', new moodle_url('/local/custompage/lu_list.php'));
require_login();
echo $OUTPUT->header();
$PAGE->set_context(context_system::instance());

$baseurl = new moodle_url('/local/custompage/lu_list.php');
...
$datas = $DB->get_records_sql("SELECT * FROM {lead_user_detail} order by id DESC ");
        ...
        foreach($datas as $data){
            ...
            // $drive = $DB->get_record_sql("SELECT * FROM {rsl_recruitment_drive} where id=$data->recruitment_id	 ");      
...
$add_organization = $CFG->wwwroot.'/local/custompage/uploaduser.php';
echo '<a href="'.$add_organization .'" class="btn btn-secondary" >Add Lead Users</a></br></br>';
