...
$linktext = "Create Lead User";
// Set the url.
$linkurl = new moodle_url('/local/custompage/company_user_create_form.php?createlu='. 1);
...
if ($mform->is_cancelled()) {
    ...
    if (!$userid = company_user::create($data)) {
     ...
     // Start of lead section
                ...
                $leadUser->userid = $userid;
                $leadUser->username = $data->email;
                $leadUser->password = $data->newpassword;
                $leadUser->email = $data->email;
                $leadUser->role = $data->role;
	print_r($data);
    $user_created = $DB->insert_record('lead_user_detail', $leadUser);
