...
require_once('btuserreport_form.php');
global $DB,$USER,$COURSE,$CFG,$PAGE,$OUTPUT;

$PAGE->set_title("BT User Report");
$PAGE->set_heading('BT User Report'); 
$PAGE->navbar->add('BT User Report', new moodle_url($CFG->wwwroot.'/local/custompage/btuserreport.php'));

$drive_id = optional_param('drive_id', '', PARAM_RAW);

$page = optional_param('page', 0, PARAM_INT);
$perpage = optional_param('perpage', 10, PARAM_INT);
$PAGE->set_url('/local/custompage/btuserreport.php', array('drive_id' => $drive_id));
$data='';
require_login();
...
if($drive_id != ""){
    ...
    $proctoring =$DB->get_record_sql("SELECT eproctoringrequired From {quizaccess_eproctoring} ep join {bt_recruitment_drive} ud where ud.test = ep.quizid and ud.id=$drive_id")->eproctoringrequired;
    // $proctoring =$DB->get_record_sql("SELECT proctoring From {test_questioncategory} tq join {bt_recruitment_drive} bd where bd.test = tq.test_id and bd.id=$drive_id")->proctoring;
    ...
    //  print_r($enrolled_users);exit; 
        foreach($datas as $key => $value){
            ...
            if($enrolled_users){
                ...
                $data .= '<td style="text-align:center">'.$enrolled_users->lastname.' </td>';
                    foreach($userheading as $key1 => $val){
                        $userdata = $DB->get_record_sql("SELECT d.data FROM {user_info_data} d  where d.fieldid = $val->id and d.userid = $value->id")->data;
                        // print($userdata);
                        if($val->id && $userdata){
            ...
            if($enrolled_users){
                ...
                if($proctoring == 1){
                    $voicedata = $DB->get_record_sql("SELECT * from {proctoringvoicewindow} where userid = $value->id and quizid = $enrolled_users->cmid ");
		            $mouth_open_count = $DB->get_record_sql("SELECT count(mouth_open_count) as total_count from {proctoringdetails} where userid = $value->id and quizid = $enrolled_users->cmid and mouth_open_count = 1 ");
                    $mobile_phone_count = $DB->get_record_sql("SELECT count(mobile_phone_count) as total_count from {proctoringdetails} where userid = $value->id and quizid = $enrolled_users->cmid and mobile_phone_count = 1 ");
                    $more_person = $DB->get_record_sql("SELECT count(more_person) as total_count from {proctoringdetails} where userid = $value->id and quizid = $enrolled_users->cmid and more_person = 1 ");
                    $no_person = $DB->get_record_sql("SELECT count(no_person) as total_count from {proctoringdetails} where userid = $value->id and quizid = $enrolled_users->cmid and no_person = 1 ");
                    $head_up = $DB->get_record_sql("SELECT count(head_up) as total_count from {proctoringdetails} where userid = $value->id and quizid = $enrolled_users->cmid and head_up = 1 ");
                    $head_down = $DB->get_record_sql("SELECT count(head_down) as total_count from {proctoringdetails} where userid = $value->id and quizid = $enrolled_users->cmid and head_down = 1 ");
                    $head_left = $DB->get_record_sql("SELECT count(head_left) as total_count from {proctoringdetails} where userid = $value->id and quizid = $enrolled_users->cmid and head_left = 1 ");
                    $head_right = $DB->get_record_sql("SELECT count(head_right) as total_count from {proctoringdetails} where userid = $value->id and quizid = $enrolled_users->cmid and head_right = 1 ");
                    $face_recognition = $DB->get_record_sql("SELECT count(face_recognition) as total_count from {proctoringdetails} where userid = $value->id and quizid = $enrolled_users->cmid and face_recognition = 1 ");
