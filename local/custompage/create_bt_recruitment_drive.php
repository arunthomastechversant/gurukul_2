...
class create_bt_recruitment_drive_form extends moodleform {
...
global $DB,$CFG,$PAGE,$USER;
		...
		$cate = $DB->get_records_sql("SELECT * FROM {quiz} WHERE course = $course->id");
		...
		//   $mform->addRule('startdate', get_string('required'), 'required', null, 'client');
		  ...
		  $select = $mform->addElement('select', 'test', 'Select Test', $cat);
