...
require_once($CFG->dirroot.'/local/custompage/itracquserupload_form.php');
...
$linktext = "Upload iTracQ User";
// Set the url.
$linkurl = new moodle_url('/local/custompage/itracquserupload.php');
...
$returnurl = $CFG->wwwroot."/local/custompage/itracquserupload.php";
...
$stdfields = array('id', 'firstname', 'lastname', 'username', 'email',
        ...
        'password', 'temppassword', 'suspended','role','itracqid','drive','highest_qualification');

$prffields = array();
if ($proffields = $DB->get_records('user_info_field',array('categoryid' => $companyid))) {
    foreach ($proffields as $proffield) {
        $prffields[] = 'profile_field_'.$proffield->shortname;
    }
    //print_r($prffields);exit();
    unset($proffields);
}
if (empty($iid)) {
    ...
    if (!empty($formdata->submitbutton)) {
        ...
        while ($line = $cir->next()) {
            ...
            if ($existinguser) {
                ...
                if($user->drive !=""){
                    $drive = $DB->get_record('rsl_recruitment_drive', array('name' => $user->drive))->id;
                    if($drive != ""){
                        $user->firstname = 'RSL';
                        $user->lastname = time();
                        $user->email= $user->profile_field_rslrslemail; 
                        // print_r($user);exit();
                        $user->id = company_user::create($user);
                        profile_save_data($user);
                        $userdetail=$DB->get_record_sql("select *  from {user} where id =$user->id");
                        $drive_data=$DB->get_record_sql("select *  from {rsl_recruitment_drive} where id =$drive");
                        $user1 = $DB->get_record('user', array('id' => $user->id, 'deleted' => 0), '*', MUST_EXIST);
                        $enrol = enrol_get_plugin('manual');
                            ...
                            foreach ($course_id as $key => $course){
                                ...
                                $end_date = strtotime("+$nodays day", $drive_data->startdate);
                                $enrol->enrol_user($instance, $user->id, 5,$drive_data->startdate,$end_date);
                            }
                    
                            if (!groups_add_member($drive_data->test_groupid, $user1->id)) {
                                print_error('erroraddremoveuser', 'group', $returnurl);
                            }

        // Start of rsl section
                        $rsluser = new stdClass();
                        $rsluser->userid = $user->id;
                        $rsluser->username =  $user->username;		
                        $rsluser->password = $user->newpassword;	
                        $rsluser->companyid = $companyid;
                        $rsluser->recruitment_id = $drive; 
                        $rsluser->timestamp =  time();
                        $rsluser->test_groupid =  $drive_data->test_groupid;
                        $rsluser->interview_id=  '';
                        $rsluser->rsl_due=  '';
                        $rsluser->resume_status = 'Pending';
                        $rsluser->itracqid = $user->itracqid;
                        $lastinsertid = $DB->insert_record('rsl_user_detail', $rsluser);

                        $record2 = new stdClass();
                        $record2->userid = $user->id;
                        $record2->recruitment_id =  $drive;	
        ...
        //  End of rsl section
            ...
            if($user->id){
                ...
                $urlto = $CFG->wwwroot.'/local/custompage/itracquserupload.php';
                redirect($urlto, 'Some Datas are Misiing ', 8);
            }
        }
...
while ($fields = $cir->next()) {
    ...
    $validemail = validate_email($rowcols['profile_field_rslrslemail']);
