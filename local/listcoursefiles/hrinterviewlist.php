...
$PAGE->set_title('HR Interview List');
$PAGE->set_heading('HR Interview List');

$PAGE->navbar->add('HR Interview List', new moodle_url('/local/listcoursefiles/hrinterviewlist.php'));


// userid
$r_user = $DB->get_records_sql("SELECT i.*,u.username,u.firstname,u.lastname ,u.email,u.deleted,u.suspended,rd.name FROM {hrinterview} i  JOIN {rsl_recruitment_drive} rd ON rd.id=i.driveid JOIN {user} u ON u.id = i.userid where i.hrid= $USER->id");


// $r_user = $DB->get_records_sql("SELECT i.*,u.username,u.firstname,u.lastname ,u.email,u.deleted,u.suspended,rd.name FROM {hrinterview} i  JOIN {rsl_recruitment_drive} rd ON rd.id=i.driveid JOIN {user} u ON u.id = i.userid where i.hrid= 329");
...
//  ----------------
    ...
    $urlto = $CFG->wwwroot.'/local/listcoursefiles/hrinterview_form.php?userid='.$userdata->userid;
$takeinterview="<a href='".$urlto."'>Manage Interview</a>";
        if($userdata->remark){
    ...
    $tplfile->interviewstatus=$userdata->status; 
