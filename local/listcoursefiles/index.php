...
if($bulkactionid == 1 && $interviewerid){
    ...
    foreach ($chosenfiles as $key => $val) {
        ...
        if(in_array($userStatus, $statusdata))
        {
...
// print_r($final_result);exit;
		...
		if($final_result){
            ...
            $email = $DB->get_record('user_info_data',array('fieldid' => 80,'userid' => $key))->data;
            $courses   = $courses;
            $search    = array("{{firstname}}", "{{courses}}", "{{username}}", "{{password}}");
            $replace   = array($firstname, $courses , $userdetails->username, $userdetails->password );
