...
//$datas = $DB->get_record_sql("SELECT * FROM {rsl_recruitment_drive} where id=$drive_id");
    ...
    foreach ($idlist as $userdata) {
        ...
        if($enrolled_users){
            $data .= '<tr>';
            $data .= '<td style="text-align:center"><input type="checkbox" class="userid" id= ' .$userdata. '></td>';
                ...
                if($proctoring == 1){
                    ...
                    $proctoringdata = $DB->get_records_sql("SELECT * from {proctoringdetails} where userid = $userdata and quizid = $enrolled_users->quizid ");
