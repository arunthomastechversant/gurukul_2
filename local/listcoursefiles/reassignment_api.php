...
if($bulkactionid == 1 ){
    ...
    $drivedate = $DB->get_record('rsl_recruitment_drive',array('id' => $courseid))->startdate;
    $search    = array("{{firstname}}","{{drivedate}}");
    $replace   = array($firstname,$drivedate);
    $mailid = $DB->get_record('user',array('id' => $interviewerid))->email;
    $maildata = $DB->get_record('mail_templates', array('company_id' =>1 ,'type_id' => 2));
    ...
    $data = array("from" => "adarsh@techversantinfo.com", "to"=> $mailid, "subject" => $subject, "isHtml" => "true", "text" => $mailcontent, "isProd" => "true");
