...
function quiz_save_best_grade($quiz, $userid = null, $attempts = array()) {
    ...
    $bestgradeval=$bestgrade*10;


    // Bk
    
    if($rsluser=$DB->get_record('rsl_user_detail', array('userid' => $USER->id))){
        if (!$userstatus = $DB->get_record('userstatus', array('userstatus' => 'Test Completed', 'userid' => $USER->id))) {
            $drive=$DB->get_record('rsl_user_detail', array('userid' => $USER->id));
            $recruitment=$DB->get_record('rsl_recruitment_drive', array('id' => $drive->recruitment_id));
            // print_r($recruitment);exit;
            if($recruitment->interview == 1){
                if($bestgradeval < 50){
                    $record2 = new stdClass();
                    $record2->userid =  $USER->id;
                    $record2->recruitment_id =  $drive->recruitment_id;	
                    $record2->userstatus =  'Rejected';	
                    ...
                    $record2->userid =  $USER->id;
                    $record2->recruitment_id =  $drive->recruitment_id;	
                    $record2->userstatus =  'Test Completed';	
                    $record2->timestamp =  time();
                    $DB->insert_record('userstatus', $record2);	
                }
            } else if($recruitment->interview == 0){
                if($bestgradeval < 50){
                    $record2 = new stdClass();
                    $record2->userid =  $USER->id;
                    $record2->recruitment_id =  $drive->recruitment_id;	
                    ...
                    $record2->timestamp =  time();	
                    $DB->insert_record('userstatus', $record2);	
                }elseif( $bestgradeval > 70){
                    $record2 = new stdClass();
                    $record2->userid =  $USER->id;
                    $record2->recruitment_id =  $drive->recruitment_id;	
                    $record2->userstatus =  'Waiting For HR Interview';	
                    $record2->timestamp =  time();
                    $DB->insert_record('userstatus', $record2);	
                }
                elseif($bestgradeval >= 50 && $bestgrade <= 70 &&  $bestgrade < 71){
                    $record2 = new stdClass();
                    $record2->userid =  $USER->id;
                    $record2->recruitment_id =  $drive->recruitment_id;	
                    $record2->userstatus =  'RSL Candidate';	
                    $record2->timestamp =  time();	
                    $DB->insert_record('userstatus', $record2);	
                }
