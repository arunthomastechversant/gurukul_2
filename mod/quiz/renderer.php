...
class mod_quiz_renderer extends plugin_renderer_base {
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        // echo "SELECT eproctoringrequired from {quizaccess_eproctoring} where quizid = $attempt->quiz";
    ...
    public function view_page($course, $quiz, $cm, $context, $viewobj) {
        ...
        // BK
        // $output .= '<video  width="500px" height="500px" autoplay ></video>
        // <div class="recorder">
        //         <span id="status">Checking background noise.</span>
        //         <!-- Volume bar -->
        //         <div id="volumeBar">
    ...
    public function view_information($quiz, $cm, $context, $messages) {
        ...
        //$ctry_data = $DB->get_record_sql("SELECT questions FROM {rsl_recruitment_drive} as brd join {test_questioncategory} as tq where tq.test_id = $quiz->id")->questions;
...
// ------------------------
        ...
        $sql = "SELECT q.id,q.qtype FROM {$CFG->prefix}quiz_slots as qs
		left join {$CFG->prefix}question as q on q.id = qs.questionid
...
// -------------------------
	//print_r($messages);
        ...
        if (count($messages) >= 3) {
            ...
            $output .='<div class="imp-div"><h4><i class="icon fa slicon-exclamation text-danger fa-fw " title="Required" aria-label="Required"></i> Important Instructions and Guidelines</h4>';
            $output .='<ul class="imp-ul">';
            $output .='<li class="imp-li">The assessment has total '.$count_questions.' questions for which the total time allowed is '.$time.' minutes.</li>';
            $output .='<li class="imp-li">The assessment has four sections. Please attempt all four sections before submitting.</li>';
            $output .='<li class="imp-li">For the entire duration of the assessment, please remain seated in front of your webcam.</li>';
            $output .='<li class="imp-li">If you get caught practicing any means of malpractice, you would be logged out of the assessment by the remote proctor.</li>';
            $output .='<li class="imp-li">Ensure proper lighting in the room – Source of light must not be behind you.</li>';
            $output .='<li class="imp-li">Please sit in a quiet room with no background noise or people around.</li>';
            $output .='<li class="imp-li">Please ensure the wall behind you has a plain background.</li>';
            $output .='<li class="imp-li">If your system shuts down abruptly, then please contact your respective College coordinator to resolve the issue.</li>';
            $output .='<li class="imp-li">If you lose Internet connectivity at any point in the assessment, do not panic. Continue attempting the assessment. Your answers will get automatically submitted once the Internet comes back.</li></ul></div>';
            $output .='<p class="imp-p"></br>Best Of Luck for the Assessment</p>';
	}
