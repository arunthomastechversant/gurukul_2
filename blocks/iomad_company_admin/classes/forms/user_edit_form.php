...
class user_edit_form extends \moodleform {
    ...
    public function definition() {
        ...
        $createType = optional_param('createlu', '', PARAM_INTEGER);
        $mform =& $this->_form;
        ...
        $company_shortname = $DB->get_record('company' ,array('id' => $companyid))->shortname;
        // print_r($company_shortname);exit();
        if ($company_shortname == 'lead') {
            if ($createType == 1) {
                ...
                $mform->addElement('select', 'role', 'User Role', $userType);
                $mform->addRule('role', 'Please Select User Role', 'required', null, 'client');
                $mform->addElement('hidden', 'createlu', 1);
            }
        }
