...
if ($mform->is_cancelled()) {
    ...
    if ($isadding) {
        ...
        $DB->update_record('company', $companydetails);
            ...
            // Create course for curesponding company
            $course = new stdClass();
            $course->fullname = $data->name;
            $course->shortname = $data->shortname;
            $course->selfenrol = 0;
            $course->summary_editor = '';
            $course->startdate = time();
            $course->enddate = 0;
            $course->submitbutton = 'Create course';
            $course->title = '';
            $course->description = '';
            ...
            if (!empty($company->category)) {
                $course->category = $company->category;
            } else {
                $course->category = $CFG->defaultrequestcategory;
            }
            $courseconfig = get_config('moodlecourse');
            $mergeddata = (object) array_merge((array) $courseconfig, (array) $course);
            
            // Turn on restricted modules.
            $mergeddata->restrictmodules = 1;
            $newcourse = create_course($mergeddata, $editoroptions);

            $department = $DB->get_record('department', array('company' => $companyid,'parent' => '0'))->id;
            if (!$DB->record_exists('company_course', array('companyid' => $companyid,'courseid' => $newcourse->id))) {
                $DB->insert_record('company_course', array('companyid' => $companyid,'courseid' => $newcourse->id,'departmentid' => $department));
            }
            if (!$DB->get_record('iomad_courses', array('courseid' => $newcourse->id))) {
                $DB->insert_record('iomad_courses', array('courseid' => $newcourse->id,'licensed' => false,'shared' => 0));
            }
            ...
            $quest = new stdClass();
            $quest->courseid = $newcourse->id;
            $quest->companyid = $companyid;
            $DB->insert_record("company_course_mapping", $quest);
