...
class global_navigation extends navigation_node {
    ...
    public function initialise() {
        ...
        //     }
        // }
        // $Userrole = $DB->get_records_sql("SELECT * FROM {role_assignments} where userid =$USER->id");
        $role = $DB->get_record_sql("SELECT roleid from {role_assignments} where userid = $USER->id")->roleid;
        // print_r($role);exit();
        if($role != 5){
    ...
    protected function load_for_user($user=null, $forceforcontext=false) {
        ...
        if ($issitecourse) {
            ...
            if ($issitecourse && $iscurrentuser && has_capability('moodle/user:manageownfiles', $usercontext)) {
                $url = new moodle_url('/user/files1.php');
    ...
    protected function generate_user_settings($courseid, $userid, $gstitle='usercurrentsettings') {
        ...
        if ($user->deleted) {
            if (!has_capability('moodle/user:update', $coursecontext)) {
                ...
                if ($course->id == $SITE->id) {
                    $profileurl = new moodle_url('/user/profile1.php', array('id'=>$user->id));
                } else {
